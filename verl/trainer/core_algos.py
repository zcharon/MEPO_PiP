...
@register_adv_estimator(AdvantageEstimator.MEPO)
def compute_mepo_outcome_advantage(
    token_level_rewards: torch.Tensor,
    response_mask: torch.Tensor,
    index: np.ndarray,
    domain: torch.Tensor,
    ...
    **kwargs,
    ...
    """
    Compute advantage for MEPO, operating only on Outcome reward (with only one scalar reward for each response).
    ...
    group_adv = torch.zeros_like(scores)
    for i in range(bsz):
        group_scores = torch.tensor(id2score[index[i]])
        assert len(group_scores) > 1, "MEPO needs rollout.n > 1 for group normalization."
        mean = torch.mean(group_scores)
        std = torch.std(group_scores)
        group_adv[i] = (scores[i] - mean) / (std + eps)
    ...
    for d in unique_domains:
        d_item = d.item()
        domain_mask = domain == d
        domain_rewards = scores[domain_mask]

        if len(domain_rewards) > 0:
            _, reward_counts = torch.unique(domain_rewards, return_counts=True)
            reward_probs = reward_counts.float() / len(domain_rewards)
            entropy = -torch.sum(reward_probs * torch.log(reward_probs + eps))
            domain_entropies[d_item] = entropy

    if len(unique_domains) > 1:
        gamma = max(domain_entropies.values())
        gamma = torch.where(gamma > eps, gamma, torch.tensor(1.0, device=gamma.device, dtype=gamma.dtype))  # 防止为 0


        sorted_unique_domains = sorted(domain_entropies.keys())
        entropies = torch.tensor([domain_entropies[d] for d in sorted_unique_domains])
        domain_entropies = {d: e.item() for d, e in zip(sorted_unique_domains, entropies)}
        weights = torch.nn.functional.softmax(entropies / (gamma + eps), dim=0)
        domain_weights = {d: w.item() for d, w in zip(sorted_unique_domains, weights)}

        print("###################################################")
        print("domain_entropies:", domain_entropies)
        print("scores:", scores)
        print("gamma:", gamma)
        print("domain_weights", domain_weights)
        print("###################################################")

        final_adv = torch.zeros_like(group_adv)
        for i in range(bsz):
            final_adv[i] = group_adv[i] * domain_weights[domain[i].item()]
    else:
        final_adv = group_adv
        domain_weights = {unique_domains.item(): 1.0}

    advantages = final_adv.unsqueeze(-1) * response_mask
    return advantages, advantages, domain_weights, domain_entropies, gamma.item()
